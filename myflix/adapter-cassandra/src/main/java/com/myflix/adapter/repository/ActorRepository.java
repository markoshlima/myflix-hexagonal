...
public class ActorRepository implements ActorPortOut {
	...
	public Actor save(Actor actor) {
		...
		new CassandraConfig().getSession().execute(actorInsert, actor.getIdActor(), actor.getName(), actor.getBirthDate());
		return actor;
	}

	@Override
	public void delete(UUID actorId) {
		new CassandraConfig().getSession().execute(actorDelete, actorId);		
	}

	@Override
	public List<Actor> getAll() {
		List<Actor> actors = new ArrayList<Actor>();
        ResultSet rs = new CassandraConfig().getSession().execute(actorGetAll);
        for(Row row : rs) {
	...
	public Actor getById(UUID actorId) {
		Actor actor = new Actor();
        ResultSet rs = new CassandraConfig().getSession().execute(actorGet, actorId);
        Row row = rs.one();
        if(row != null) {
        	actor.setIdActor(row.getUuid("id_actor"));
            actor.setName(row.getString("name"));
